...
path = '/afs/cern.ch/user/e/efe/workspace_afs/GENprod13/genproductions/bin/MadGraph5_aMCatNLO/cards/production/2017/13TeV/g2HDM/'
#path = '/eos/cms/store/group/phys_generator/cvmfs/gridpacks/UL/13TeV/madgraph/V5_2.6.5/g2HDM/ttc/'
#files = os.listdir(path)
files = glob.glob("/afs/cern.ch/user/e/efe/workspace_afs/GENprod13/genproductions/bin/MadGraph5_aMCatNLO/cards/production/2017/13TeV/g2HDM/ttc_a0_M*_s0_M*")
bash_com = '#!/bin/sh'
cd_com = 'cd /afs/cern.ch/user/e/efe/workspace_afs/GENprod13/genproductions/bin/MadGraph5_aMCatNLO/'
...
#    if a0_mass < 999  or a0_mass > 1000 or "50_s0" in fname or 'a0_M1000_rhotu00_rhotc10_rhott00' not in fname:
    if "a0_M000" not in fname or "s0_M200" not in fname or "rhotc04" not in fname:
        continue
    fw = open(fname+'_sub.sh','w')
    fw.write(bash_com+'\n')
    fw.write(cd_com+'\n')
    fw.write(main_com+' g2HDM_'+fname+' cards/production/2017/13TeV/g2HDM/'+fname+'\n')
    fw.write('mv g2HDM_'+fname+'*.tar.xz /eos/cms/store/group/phys_generator/cvmfs/gridpacks/UL/13TeV/madgraph/V5_2.6.5/g2HDM/ttc_interference/.')
    fw.close()
    ...
    cond_submit.write('requirements = (OpSysAndVer =?= "CentOS8")'+'\n')
#   espresso     = 20 minutes
#   microcentury = 1 hour
#   longlunch    = 2 hours
#   workday      = 8 hours
