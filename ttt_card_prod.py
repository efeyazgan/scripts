...
for ind in mass:
    ...
    f_name_extramodels = f_name+'/g2HDM_'+f_name+'_extramodels.dat'
    f_name_run_card = f_name+'/g2HDM_'+f_name+'_run_card.dat'
    os.system('cp ttt/g2HDM_ttt_a0_extramodels.dat '+f_name_extramodels)
    os.system('cp ttt/g2HDM_ttt_a0_run_card.dat '+f_name_run_card)
    ...
    with open(f_name_cust,'w') as cust_file:
        for el in cust:
            print >> cust_file, el
    lines = []
    with open(f_name_run_card) as run_card_file:  
        for line in run_card_file:
            line = line.replace("6500","6800")
            lines.append(line)
    with open(f_name_run_card,'w') as runcard_out:
        for line in lines:
            runcard_out.write(line)
